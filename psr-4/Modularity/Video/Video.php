...
namespace MunicipioExtended\Modularity\Video;
...
class Video extends \Modularity\Module {
  ...
  private function readRemoteFile($url) {
    ...
    if (!is_wp_error($responseHandle)) {
      ...
      $responseMime = wp_remote_retrieve_header(
        $responseHandle,
        "content-type",
      );
      ...
      if (
        $responseCode == 200 &&
        in_array($responseMime, ["image/jpeg", "image/jpg"])
      ) {
        return $responseBody;
      }
    }
  ...
  private function storeImage($fileContent, $videoId) {
    ...
    //Explicit path
    $fullPath = implode("/", [$uploadsDir, $uploadsSubDir, $fileName]);

    //Relative to uploads dir
    $subPath = implode("/", [$uploadsSubDir, $fileName]);
  ...
  public function data(): array {
    ...
    $data["image"] = false;
    if (
      isset($data["placeholder_image"]) &&
      !empty($data["placeholder_image"])
    ) {
