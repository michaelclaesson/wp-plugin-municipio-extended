  ...
  <section class="o-container t-customsearchpage u-margin__top--6">
    <div class="o-grid">
      <div class="o-grid-12 tailwind">
        ...
        <mx-site-search class="group space-y-10" search-callback="mxSearch" data-admin-url="{{ admin_url('admin-ajax.php') }}" data-nonce="{{ wp_create_nonce('mx_search') }}">
          <form part="form" id="site-search-form">
            <div class="grid grid-cols-[1fr_max-content] items-stretch h-12">
              <input
                ...
                class="appearance-none bg-white font-inherit border-none p-4 w-full"
              />
              <button type="submit" class="bg-primary hover:bg-primary-dark border-none text-white py-4 px-6 cursor-pointer">Sök</button>
            </div>
          </form>
          ...
          <div class="space-y-10 scroll-my-5 focus:outline-none" part="results" tabindex="-1">
            ...
            >
              ...
              <ol class="list-none p-0 space-y-4" aria-labelledby="mx-site-search-results-heading">
                <template name="hit">
                  <li>
                    <a href="" class="flex text-inherit no-underline bg-white p-6 [&_mark]:bg-complementary-lighter hover:text-inherit visited:hover:text-inherit group/hit gap-6" slot="link">
                      <div class="flex-grow space-y-2">
                        <h3 slot="title" class="m-0 decoration-1 group-hover/hit:underline"></h3>
            ...
            <div
              class="hidden group-[:state(pending):not(:state(has-results))]:block"
            >
            ...
            <mx-pagination value="{{ get_query_var('paged') ?: 1 }}" max="{{ get_query_var('paged') ?: 1 }}" part="pagination" class="hidden [&:state(has-options)]:block" aria-label="Sidor">
              <ul class="list-none p-0 flex gap-2 items-center justify-center">
                <template name="item">
                  <li>
                    <button slot="button" class="aria-[current=page]:bg-primary aria-[current=page]:text-white aria-[current=page]:hover:bg-primary-dark border-none bg-white py-1 px-2 min-w-8 hover:bg-primary hover:text-white cursor-pointer text-center text-lg"></button>
                  </li>
                </template>
