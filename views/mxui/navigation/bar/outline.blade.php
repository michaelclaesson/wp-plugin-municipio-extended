<div class="o-container">
  <ul class="grid grid-cols-[repeat(auto-fit,minmax(min(100%,7rem),10rem))] gap-4 space-y-0 mx-auto justify-center">
    @foreach($items as $item)
      <li class="contents">
        <a href="{{ $item['href'] }}" class="border border-primary hover:border-black text-link hover:text-black grid grid-rows-subgrid row-span-2 gap-2 items-center visited:hover:text-black text-center p-3 hover:bg-[#0001] rounded-lg transition-colors">
