...
function check_and_install_language() {
    ...
    if (!file_exists($languages_dir)) {
        if (!mkdir($languages_dir, 0755, true)) {
            error_log('Failed to create languages directory: ' . $languages_dir . ' - ' . __FILE__ . ' ' . __LINE__);
            return;
        }
    }
    ...
    if (!file_exists($languages_dir . '/sv_SE.mo')) {
        ...
        } elseif (file_exists('/opt/homebrew/bin/php') && file_exists('/opt/homebrew/bin/wp')) {
            // Mac laptop environment
            $php_path = '/opt/homebrew/bin/php';
            $wp_cli_path = '/opt/homebrew/bin/wp';
        } else {
            error_log('PHP or WP-CLI binary not found. Cannot proceed with language installation.');
            return;
        }

        // Run the command to install the language
        exec("$php_path $wp_cli_path language core install sv_SE", $output, $return_var);
        if ($return_var !== 0) {
            error_log('$return_var: ' . $return_var . ' - ' . __FILE__ . ' ' . __LINE__);
            error_log('$output: ' . print_r($output, true) . ' - ' . __FILE__ . ' ' . __LINE__);
        }
    }
}
